...
export default function PriceSearch({ setSearchTerms }) {
  ...
  return (
    <div className="w-full md:w-1/2 lg:w-1/4 relative">
      ...
      {showRange && (
        <div className="absolute top-[80%] right-0 w-full px-8 pb-4 pt-9 shadow-lg rounded-md flex flex-col items-center justify-between h-32 bg-white z-[10000] m-4 mt-5 [&>div>span>span]:font-poppins [&>div>span>span]:text-xs [&>div>span>span]:font-bold [&>div>div>span>span]:font-poppins [&>div>div>span>span]:text-xs [&>div>div>span>span]:hidden">
          <InputRange
            maxValue={maxPrice}
            minValue={minPrice}
            value={value}
          ...
          <button
            onClick={() => {
              ...
              setSearchTerms((prev) => ({
                ...prev,
                maxPrice: parseInt(value.max),
                minPrice: parseInt(value.min),
              }));
            }}
